...
export const globRenderer: ToolRenderer = {
  renderCall(ctx: ToolRenderContext) {
    ...
    return new Text(
      `${callBullet(ctx)} ${toolTitle("glob")} ${theme.yellow(pattern)}  ${theme.muted(path)}`,
      1,
      0,
    );
  },
  renderResult(result, _opts, _ctx) {
    if (result.isError) {
      return new Text(`${statusBullet("error")} ${result.content.slice(0, 200)}`, 1, 0);
    }
    ...
    return new Text(
      `${statusBullet("success")} ${theme.muted(`${files.length} file${files.length === 1 ? "" : "s"}`)}` +
        (head ? `\n      ${head}${more}` : ""),
      1,
      0,
    );
